...
public class FollowServiceImpl implements FollowService {
    ...
    public List<String> getFollowingUserIds(String userId) {
        User follower = userRepository.findById(userId).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));

        List<Follow> follows = followRepository.findAllByFollower(follower);

        // 🔥 Trả về danh sách `followed` userId
        return follows.stream().map(f -> f.getFollowed().getId()).collect(Collectors.toList());
    }
